...
import SkyHighNano from "../../lib/skyhigh-nano";
 ...
 * it into the console
 *
 * @param {SkyHighNano} bridge
 * @returns {Promise<void>}
 */
async function showVersion(bridge) {
    // inform
    console.log("\nRequesting SkyHigh Nano Ledger app version.");

    // get the version info
    const ver = await bridge.getVersion();
    console.log(`    > SkyHigh Nano app version is: ${ver.major}.${ver.minor}.${ver.patch}`);
...
async function getSignedTransaction(bridge) {
    ...
    const tx = {
        ...
        value: "0xde0b6b3a7640000", /* 1 SKH in WEI (1e+18) in HEX */
...
async function run() {
    // inform
    console.log("-----------------------------------------------");
    console.log("SkyHigh Nano Ledger application example started.");
    console.log("-----------------------------------------------");
    ...
    // init the API bridge
    const bridge = new SkyHighNano(tr);
    try {
