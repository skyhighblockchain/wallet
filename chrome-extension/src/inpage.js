 ...
 */
class SkyHighInpageProvider extends SkyHighEventEmitter {
    ...
    _sendToContentScript(payload, callback = null) {
        ...
        if (this.debug) console.log('SkyHighPwaWallet request', id, payload);
        window.postMessage({
            target: 'SkyHighPWAwalletBackground',
            data: payload,
            msgId: id,
        }, location.origin);
    }
    ...
    _receiveFromContentScript(event) {
        ...
        if (msg.target !== 'SkyHighPWAwalletInpage') return
        if (!msg.data) return
        let data = msg.data;

        if (this.debug) console.log('SkyHighPwaWallet response', msg.msgId, data);
        ...
        if (msg.msgId && typeof this.messageCallbacks[msg.msgId] !== 'undefined') {
            ...
            if (data.error) {
                console.log('SkyHighPwaWallet RPC error', data);
                callback(data.error, null); // (error, result)
            } else {
                callback(null, data); // (error, result)
            }
            if (this.debug) console.log('SkyHighPwaWallet response callback called', msg.msgId, data);
        }
    ...
    _handleAccountsChanged (accounts, isEthAccounts = false, isInternal = false) {
        if (this.debug) console.log('SkyHighPwaWallet accountsChanged', accounts);
        this.emit('accountsChanged', accounts)
    }
}

window.ethereum = new SkyHighInpageProvider();
...
console.log("SkyHigh-PWA-Wallet initialized");
