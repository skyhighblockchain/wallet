<template>
    <div class="defi-mint-sskh-confirmation">
        <tx-confirmation
            v-if="hasCorrectParams"
            :tx="tx"
            send-button-label="Submit"
            password-label="Please enter your wallet password to mint sSKH"
        ...
        >
            <h2 class="not-visible" data-focus>Mint sSKH - Confirmation</h2>
...
export default {
    ...
    methods: {
        async setTx() {
            ...
            this.dAmountDelegated = this.$fWallet.WEIToSKH(this.d_amountDelegated);
        ...
        onSendTransactionSuccess(_data) {
            if (!this.isView) {
                this.$emit('change-component', {
                    to: 'staking-mint-sskh-confirmation-success-message',
                    from: this.compName,
                    data: {
                ...
                this.$router.replace({
                    name: `defi-mint-sskh-transaction-success-message`,
                    params,
                });
            }
        },
        ...
        onChangeComponent(_data) {
            let transactionRejectComp = `defi-mint-sskh-transaction-reject-message`;
