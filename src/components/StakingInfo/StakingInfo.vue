<template>
    <div ref="doc" class="stake-skh" tabindex="0">
                ...
                <div class="col">
                        ...
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    <s-k-h-token-value :value="accountInfo.amountDelegated" convert />
                                </template>
                            </f-placeholder>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    <s-k-h-token-value :value="accountInfo.pendingRewards" convert />
                                </template>
                            </f-placeholder>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    -
                                    <!--{{ toSKH(accountInfo.claimedRewards) }} SKH-->
                                </template>
                            </f-placeholder>
                        </div>
                    </div>
                    <div class="row no-collapse">
                        <div class="col f-row-label">Minted sSKH</div>
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    <s-k-h-token-value :value="outstandingSSKH" convert no-currency />
                                    sSKH
                                </template>
                            </f-placeholder>
                        </div>
                    ...
                    <div class="row no-collapse">
                        ...
                        <div class="col">
                            <f-placeholder :content-loaded="!!accountInfo" block :replacement-num-chars="10">
                                <template v-if="accountInfo">
                                    <s-k-h-token-value :value="accountInfo.pendingWithdraw" convert />
                                </template>
                            </f-placeholder>
                        </div>
                    </div>
                        ...
                        <template v-if="accountInfo && accountInfo.delegated !== '0x0'">
                            <template v-if="accountInfo && accountInfo.preparedForWithdrawal && false">
                                ...
                                <h3 class="align-center">
                                    Your {{ toSKH(accountInfo.delegated) }} Skyhigh SKH is available for withdraw in 7 days.
                                </h3>
                                -->
                            </template>
                            <template v-else>
                                ...
                                <f-message v-if="showRepaySSKHMessage" type="info" with-icon class="align-start">
                                    Can't repay sSKH, not enough unlocked sSKH
                                </f-message>
                                <f-message v-if="showUndelegateMessage" type="info" with-icon class="align-start">
                                    Can't undelegate, repay sSKH first please
                                </f-message>
                            </template>
                        </template>
                        <!--                        <template v-else>
        ...
        >
            ...
            <f-message
                v-if="outstandingSSKH > 0"
                type="warning"
                with-icon
                class="align-start"
                style="margin-bottom: 16px;"
            >
                Can't withdraw, repay sSKH first please
            </f-message>

            <withdraw-request-list
                :disable-withdraw="outstandingSSKH > 0"
...
import { toSKH, WeiToSKH } from '../../utils/transactions.js';
...
import { SFC_CLAIM_MAX_EPOCHS } from '@/plugins/skyhigh-web3-wallet.js';
...
*/
        ...
        canRepaySSKH() {
            return (
                // this.canUndelegate &&
                this._delegation &&
                this._delegation.outstandingSSKH !== '0x0' &&
                this.outstandingSSKH <= this.availableSSKH
            );
        },
        ...
        outstandingSSKH() {
            return this.sskhToken && this._delegation
                ? this.$defi.fromTokenValue(this._delegation.outstandingSSKH, this.sskhToken) || 0
                : 0;
        },
        ...
        withdrawRequestsAmount() {
            ...
            if (delegation && delegation.withdrawRequests && delegation.withdrawRequests.length) {
                delegation.withdrawRequests.forEach((_request) => {
                    if (delegation.toStakerId === _request.stakerID) {
                        amount += WeiToSKH(_request.amount);
                    }
                });
            }
    ...
    methods: {
        ...
        mintSSKH() {
            ...
            this.showConfirmationWindow({
                compName: 'defi-mint-s-s-k-h-confirmation',
                data: {
                    stakerId: this.d_stakerId,
                    amountDelegated: this._delegation.amountDelegated,
                    // stakerAddress: stakerInfo ? stakerInfo.stakerAddress : '',
                },
                windowTitle: 'Mint sSKH',
            });
        },

        repaySSKH() {
            if (!this.canRepaySSKH) {
                return;
            }

            this.showConfirmationWindow({
                compName: 'defi-repay-s-s-k-h-confirmation',
                data: {
                    stakerId: this.d_stakerId,
                    outstandingSSKH: this._delegation.outstandingSSKH,
                    // stakerAddress: stakerInfo ? stakerInfo.stakerAddress : '',
                },
                stepsCount: 2,
                steps: ['Allow', 'Confirm', 'Finished'],
                windowTitle: 'Repay sSKH',
            });

            /*this.$emit('change-component', {
                to: 'defi-repay-s-s-k-h-confirmation',
                from: 'staking-info',
                data: {
                    stakerId: this.d_stakerId,
                    outstandingSSKH: this._delegation.outstandingSSKH,
        ...
        async onWithdrawRequestSelected(_withdrawRequest) {
            ...
            this.showConfirmationWindow({
                compName: 'withdraw-s-k-h-confirmation',
                data: {
                    accountInfo: {
                        ...accountInfo,
                        stakerInfo,
                    },
                    amount: WeiToSKH(_withdrawRequest.amount),
                    withdraw: true,
                    withdrawRequest: _withdrawRequest,
                    stakerId: this.d_stakerId,
                },
                windowTitle: 'Withdraw delegated SKH',
            });

            /*this.$emit('change-component', {
                to: 'withdraw-s-k-h-confirmation',
                from: 'staking-info',
                data: {
                    accountInfo: {
                        ...accountInfo,
