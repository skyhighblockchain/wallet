        ...
        <template v-if="success">
            ...
            <ul class="no-markers" style="margin-bottom: 16px;">
                <li v-for="result in tsData.result" :key="result.uuid">
                    <b>
                        {{ result.amount }}
                        {{ $bnb.getSKHCurrencyByDirection(receive ? tsData.direction : 'SkyhighToSkyhigh') }}
                    </b>
                    from
                    <b class="break-word">{{
                        receive ? tsData[$bnb.getAddressKeyByDirection(tsData.direction)] : tsData.from_skyhigh_address
                    }}</b>
                </li>
            </ul>
            ...
            <p>
                ...
                <b>
                    ...
                    {{ $bnb.getSKHCurrencyByDirection(receive ? 'SkyhighToSkyhigh' : tsData.direction) }}
                </b>
                in your address
                <b> {{ tsData.address || tsData.result[0].skyhigh_address }}</b>
            </p>
