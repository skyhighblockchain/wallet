    ...
    <div class="transaction-confirmation">
        ...
        >
            <h2 class="not-visible" data-focus>
                ...
                <span v-else>Send Skyhigh SKH</span>
            </h2>
            ...
            <div class="transaction-info">
                <div class="row no-collapse">
                    ...
                    <div class="col break-word">
                        ...
                        >
                            ...
                            <template v-else-if="sendToAddressBalance"> {{ sendToAddressBalance }} SKH </template>
                ...
                <template v-if="sendDirection !== 'SkyhighToSkyhigh'">
                    <f-message type="warning" class="align-center">
                        All bridge transactions incur a fee of {{ minSKHToTransfer }} SKH, deducted from the transfer
                        amount.
                    </f-message>
                    <f-message type="info" class="big">
                        You will receive <b>{{ txData.amount - minSKHToTransfer }} SKH</b>
                    </f-message>
                    <br />
                </template>
            ...
            <template #window-content>
                        ...
                        <div class="row no-collapse">
                            ...
                            <div class="col break-word">
                                ...
                                <span class="f-row-label">
                                    ...
                                    ( {{ toSKH(currentAccount.balance) }} SKH
                                    <template v-if="currentAccount.name">, {{ currentAccount.name }}</template> )
                                </span>
                            </div>
                        </div>
                ...
                <f-message v-if="sendDirection === 'SkyhighToEthereum'" type="warning" class="align-center">
                    All bridge transactions incur a fee of {{ minSKHToTransfer }} SKH, deducted from the transfer
                    amount.
                    <br />
                    You will receive {{ txData.amount - minSKHToTransfer }} SKH
                    <br />
                </f-message>
            </template>
        </tx-confirmation>
...
import { Web3 } from '../../plugins/skyhigh-web3-wallet.js';
import { toSKH } from '../../utils/transactions.js';
...
export default {
    components: { SKHTokenValue, FMessage, FTokenValue, TxConfirmation },
    ...
    asyncComputed: {
        async sendToAddressBalance() {
            ...
            } else if (sendDirection === 'SkyhighToSkyhigh') {
                data = await this.$fWallet.getBalance(this.txData.skyhigh_address);
                balance = this.toSKH(data.balance);
            } else if (sendDirection === 'SkyhighToBinance') {
            ...
            } else if (sendDirection === 'SkyhighToEthereum') {
    ...
    methods: {
        async setSendToAddress() {
            ...
            if (sendDirection === 'SkyhighToSkyhigh') {
                this.sendToAddress = this.txData.skyhigh_address;
            } else if (sendDirection === 'SkyhighToBinance') {
                stData = {
            ...
            if (stData) {
                ...
                this._swapTokenData = {
                    from_skyhigh_address: this.currentAccount.address,
                    ...this.txData,
                    ...data,
                };
                this.sendToAddress = data.skyhigh_address;
                this.dTxData.skyhigh_address = data.skyhigh_address;
            ...
            if (token.address) {
                this.tx = await fWallet.getDefiTransactionToSign(
                    erc20Utils.erc20TransferTx(
                        token.address,
                        fWallet.toChecksumAddress(dTxData.skyhigh_address),
                ...
                this.tx = await fWallet.getTransactionToSign({
                    ...
                    from,
                    to: fWallet.toChecksumAddress(dTxData.skyhigh_address),
                    memo: dTxData.memo,
                });
            }
        },
