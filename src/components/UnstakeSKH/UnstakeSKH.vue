<template>
    <div class="unstake-skh">
        <h2 :id="labelId" class="not-visible" data-focus>Undelegate SKH</h2>
            ...
            <f-form ref="form" center-form :aria-labelledby="labelId" @f-form-submit="onFormSubmit">
                <!--                <legend class="h2" data-focus>
                    <div class="cont-with-back-btn">
                        <span>
                            Undelegate SKH <span class="f-steps"><b>1</b> / {{ lockExist ? '3' : '2' }}</span>
                        </span>
                        <button
                            type="button"
                            class="btn light"
                ...
                <div class="form-body">
                    ...
                    <h3 v-if="lockExist" class="orange-color" style="padding-top: 0;">
                        {{ cToUnlockAmount }} SKH of {{ amount }} SKH are still locked.
                        <template v-if="cUnlockPenalty > 0">
                            You will lose {{ cUnlockPenalty }} SKH by undelegating before the lock expiration.
                        </template>
                    </h3>
                    <!--                    <h3 v-if="lockExist" class="orange-color" style="padding-top: 0;">
                        Your delegation is still locked. You will lose
                        {{ cUnlockPenalty > 0 ? `${cUnlockPenalty} SKH` : 'part of your rewards' }}
...
export default {
    ...
    computed: {
        undelegateMax() {
            return this.accountInfo ? WEIToSKH(this.accountInfo.delegation.amountDelegated) : 0;
            /*
            return this.accountInfo
                ? WEIToSKH(this.accountInfo.delegation.amount) - this.accountInfo.withdrawRequestsAmount
                : 0;
*/
        },
    ...
    methods: {
        checkAmount(_value) {
            ...
            this.amountErrMsg = `You can undelegate min 1 SKH and max ${undelegateMax} SKH`;
