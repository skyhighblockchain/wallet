                    ...
                    <div class="col break-word">
                        <template v-if="canClaimRewards(item.rewards)">
                            <f-token-value
                                ...
                                :use-placeholder="false"
                                :token="wskhToken"
                                :value="pendingRewardsWSKH(item.rewards) - stashedRewardsWSKH(item.rewards)"
                            />
                            <span class="currency-light">
                                ...
                                <f-token-value
                                    :use-placeholder="false"
                                    :token="wskhToken"
                                    :value="stashedRewardsWSKH(item.rewards)"
                                />
                            </span>
                        </template>
                ...
                <div v-if="column" class="row no-collapse no-vert-col-padding">
                    ...
                    <div class="col break-word">
                        <template v-if="usedAsCollateral(item)">
                            ...
                            <template v-if="item.symbol === 'WSKH'">
                                ,
                        ...
                        <!--                        <template v-if="canClaimRewards(item.rewards)">
                            ,<a
                                ...
                                href="#"
                                :data-pending-rewards="pendingRewardsWSKH(item.rewards)"
...
import { MAX_TOKEN_DECIMALS_IN_TABLES } from '@/plugins/skyhigh-web3-wallet.js';
...
export default {
    ...
    methods: {
        ...
        async addRows(_account) {
            ...
            const tokens = result[1].filter((_item) => _item.isActive && _item.symbol !== 'SKH');

            this.wskhToken = tokens.find((_item) => _item.symbol === 'WSKH');
            ...
            const items = tokens.filter((_item) => {
                ...
                if ((collateral !== 0 || debt !== 0) && _item.symbol !== 'WSKH' && _item.symbol !== 'SSKH') {
        ...
        usedAsCollateral(_token) {
            return _token.symbol === 'WSKH' || _token.symbol === 'SSKH';
        },
         ...
         * @param {object} _rewards
         */
        pendingRewardsWSKH(_rewards) {
            return this.$defi.fromTokenValue(_rewards.rewardsEarned, this.wskhToken) || 0;
        },

        /**
         * @param {object} _rewards
         */
        stashedRewardsWSKH(_rewards) {
            return this.$defi.fromTokenValue(_rewards.rewardsStashed, this.wskhToken) || 0;
        },
