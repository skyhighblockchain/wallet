<template>
    <div class="open-positions-list-dt">
        ...
        >
            ...
            <template v-slot:column-balance_tusd="{ value, item, column }">
                <div v-if="column" class="row no-collapse no-vert-col-padding">
                    <div class="col-6 f-row-label">{{ column.label }}</div>
                    <div class="col break-word">
                        <template v-if="item._collateral > 0">
                            depo.: <b>{{ formatCollateralTUSD(item) }}</b> <br />
                        </template>
                        <template v-if="item._debt > 0">
                            borr.: <b>{{ formatDebtTUSD(item) }}</b>
                        </template>
                    </div>
                </div>
                <template v-else>
...
export default {
    ...
    watch: {
         ...
         */
        tokens(_value) {
            const items = _value.filter((_item) => _item.isActive && _item.canDeposit && _item.symbol !== 'SKH');
