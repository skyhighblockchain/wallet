<template>
    <div class="send-transaction-form">
        <h2 :id="labelId" class="with-back-btn align-center" data-focus>
            ...
            <template v-else>Send Skyhigh SKH</template>
                ...
                <fieldset class="">
                    <div class="form-body">
                        ...
                        <address-field
                            ...
                            :validate-on-input="d_sendDirection === 'SkyhighToSkyhigh'"
                        >
                            <template #bottom="sProps">
                                <f-message v-show="sProps.showErrorMessage" type="error" alert with-icon>
                                    {{ sendToErrorMsg }}
                        ...
                        <f-input
                            v-if="d_sendDirection !== 'SkyhighToEthereum' && !token.address"
                            label="Memo (optional)"
                            field-size="large"
                            name="memo"
                        />
                        ...
                        <div class="align-center form-buttons">
                            <template v-if="d_sendDirection !== 'SkyhighToSkyhigh'">
                                <f-message type="warning" class="align-center">
                                    All bridge transactions incur a fee of {{ minSKHToTransfer }} SKH, deducted from the
                                    transfer amount.
                                </f-message>
                                <f-message
                                    v-if="amount >= minSKHToTransfer && checkAmount(amount)"
                                    type="info"
                                    class="big"
                                >
                                    You will receive <b>{{ amount - minSKHToTransfer }} SKH</b>
                                </f-message>
                                <br />
                            </template>
...
export default {
    ...
    data() {
        return {
            d_sendDirection: 'SkyhighToSkyhigh',
            amountErrMsg: 'Invalid amount',
            gasPrice: '',
            amount: '',
            sendToErrorMsg: 'Enter a valid Skyhigh SKH address or domain name',
            /** Balance of BNB or ETH account. */
            ETHOrBNBAccountBalance: '',
            minSKHToTransfer: appConfig.bnbridgeApi.minSKHToTransfer,
    ...
    computed: {
        ...
        tokenSymbol() {
            ...
            return token.address ? this.$defi.getTokenSymbol(token) : 'SKH';
        },
        ...
        sendToLabel() {
            ...
            switch (this.d_sendDirection) {
                case 'SkyhighToBinance':
                    sendTo = 'BNB Receive Address';
                    break;
                case 'SkyhighToEthereum':
    ...
    methods: {
        ...
        async checkAddress(_value) {
            ...
            if (d_sendDirection === 'SkyhighToSkyhigh') {
                value = (await this.resolveAddress(value, 'SKH', 'SKYHIGH')) || value;
                validAddress = this.$fWallet.isValidAddress(value);
                this.sendToErrorMsg = 'Enter a valid Skyhigh SKH address or domain name';
            } else if (d_sendDirection === 'SkyhighToBinance') {
                ...
                if (validAddress) {
                    if (value.toLowerCase() === 'bnb136ns6lfw4zs5hg4n85vdthaad7hq5m4gtkgf23') {
                        ...
                        try {
                            ...
                            this.ETHOrBNBAccountBalance = `Current SkyHigh Balance: ${data.balance} SKH`;
                        } catch (_error) {
                            validAddress = false;
            ...
            } else if (d_sendDirection === 'SkyhighToEthereum') {
                ...
                if (validAddress) {
                    try {
                        ...
                        this.ETHOrBNBAccountBalance = `Current SkyHigh Balance: ${balance} SKH`;
                    } catch (_error) {
                        validAddress = false;
        ...
        checkAmount(_value) {
            ...
            const { minSKHToTransfer } = this;
            const skyhighToBridge = this.d_sendDirection !== 'SkyhighToSkyhigh';
            ...
            if (!isNaN(value)) {
                if (value <= remainingBalance && value > 0) {
                    if (skyhighToBridge && value < minSKHToTransfer) {
                        this.amountErrMsg = `You must transfer at least ${minSKHToTransfer} ${tokenSymbol}`;
                    } else {
                        ok = true;
                    }
        ...
        */
        async onFormSubmit(_event) {
            ...
            if (this.currentAccount && data.amount) {
                if (d_sendDirection === 'SkyhighToSkyhigh') {
                    data.skyhigh_address = this.resolvedAddress || data.address;
                } else if (d_sendDirection === 'SkyhighToBinance') {
                ...
                this.$emit('change-component', {
                    ...
                    data: {
                        ...
                        amount: '0.1',
                        fee: '0.000044',
                        from_skyhigh_address: '0x76AE07E6D236c1aE3F5C3112F387ad82c69A2471',
                        memo: '',

                        bnb_address: 'bnb1jvlepaalght59kwhfh44k54u3elmhuurnt0lxc',
                        direction: 'SkyhighToBinance',
                        skyhigh_address: '0xE504aF2999644A86162Df892E86E3809a365AEBa',
...
*/
                ...
                /*this.$emit('change-component', {
                    ...
                    data: {
                        ...data,
                        // fee: this.$fWallet.WEIToSKH(this.$fWallet.getTransactionFee(this.gasPrice)),
                    },
                });*/
            }
        },
