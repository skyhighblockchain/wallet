<template>
    <div class="stake-form" :class="{ 'increase-delegation': d_increaseDelegation }">
        <h1 :id="labelId" data-focus class="with-back-btn align-center" aria-label="Delegate SKH">
            <span>Delegate SKH</span>
...
import { GAS_LIMITS } from '../../plugins/skyhigh-web3-wallet.js';
...
export default {
    ...
    methods: {
        ...
        checkAmount(_value) {
            ...
            if (!isNaN(value)) {
                if (value <= remainingBalance && value >= 1) {
                    ...
                    this.amountErrMsg = `You can't stake amount less than 1 SKH`;
                } else if (value >= 1) {
                    this.amountErrMsg = `You can stake max ${remainingBalance} SKH`;
                }
            }

            if (ok && this.validatorInfo.delegatedLimit && this.validatorInfo.address) {
                const delegatedLimit = parseFloat(this.$fWallet.WEIToSKH(this.validatorInfo.delegatedLimit));

                if (value > delegatedLimit) {
                    this.amountErrMsg =
                        `Staking limit reached. You can stake max ${delegatedLimit} SKH on validator ` + this.validator;
                    ok = false;
                }
            }
         ...
         *
         * @param {Number} _amount Amount of SKH to stake.
         * @return {Promise<void>}
         */
