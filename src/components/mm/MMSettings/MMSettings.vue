                ...
                <template v-if="isMMInstalled">
                    <div class="mmsettings_section">
                        ...
                        <button
                            id="add_mainnet_btn"
                            class="btn large"
                            :disabled="addSkyHighMainnetInProgress"
                            @click="onAddChainClick('mainnet')"
                        >
                            Add SkyHigh Skyhigh Mainnet
                            <pulse-loader v-if="addSkyHighMainnetInProgress" color="#fff"></pulse-loader>
                        </button>
                        <button
                            id="add_testnet_btn"
                            class="btn large secondary"
                            :disabled="addSkyHighTestnetInProgress"
                            @click="onAddChainClick('testnet')"
                        >
                            Add SkyHigh Testnet
                            <pulse-loader v-if="addSkyHighTestnetInProgress" color="#1969ff"></pulse-loader>
                        </button>
                    </div>
...
import { SKYHIGH_MAINNET, SKYHIGH_TESTNET } from '@/plugins/mm/mm.js';
...
export default {
    ...
    data() {
        return {
            ...
            popoverText: '',
            btnId: '',
            addSkyHighMainnetInProgress: false,
            addSkyHighTestnetInProgress: false,
    ...
    methods: {
        ...
        stopLoadingIndicators() {
            this.addSkyHighMainnetInProgress = false;
            this.addSkyHighTestnetInProgress = false;
        ...
        async onAddChainClick(_type = 'mainnet') {
            ...
            const chain = mainnet ? SKYHIGH_MAINNET : SKYHIGH_TESTNET;
            const btnId = mainnet ? 'add_mainnet_btn' : 'add_testnet_btn';

            if (mainnet) {
                this.addSkyHighMainnetInProgress = true;
            } else {
                this.addSkyHighTestnetInProgress = true;
            }
