...
export class BNBridgeExchange {
    ...
    /**
     * Get skyhigh token from tokens list.
     *
     * @return {BNBridgeToken|undefined}
     */
    async getSkyHighToken() {
        const tokens = await this.getTokens();

        return tokens ? tokens.find((_token) => _token.symbol === 'SKH') : undefined;
    }
    ...
    async getETHBalance(_ethReceiveAddress, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getSkyHighToken());
        const pData = {
     ...
     * @param {string} [skyhighAddress]
     * @param {BNBridgeDirection} [direction]
     * @param {BNBridgeToken} [_token]
     * @return {Promise<null>}
     */
    async swapToken({ ethAddress = '', bnbAddress = '', skyhighAddress = '', direction = 'EthereumToBinance' }, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getSkyHighToken());
        const pData = {
            direction,
            token_uuid: token ? token.uuid : '',
            ...
            skyhigh_address: skyhighAddress,
        };
            ...
            if (data) {
                if (data.success) {
                    ...
                    if (
                        ...
                        pData.skyhigh_address !== pData.skyhigh_address.toLowerCase()
                    ) {
                        // resend request with Skyhigh address in lowercase
                        pData.skyhigh_address = pData.skyhigh_address.toLowerCase();
     ...
     * @return {string}
     */
    getSKHCurrencyByDirection(_direction) {
        let currency = 'SKH';

        if (_direction === 'SkyhighToSkyhigh') {
            currency += '-Skyhigh';
