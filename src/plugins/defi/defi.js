...
export class DeFi {
    ...
    getMaxDebtTUSD(_collateralTUSD) {
        return _collateralTUSD > 0 ? _collateralTUSD / this.minCollateralRatio : 0;
    }
    ...
    getMintingLimitTUSD(_debt, _collateralTUSD) {
        // ratio between actual debt and liquidation debt
        return _collateralTUSD > 0 ? (_debt / (_collateralTUSD / this.liqCollateralRatio)) * 100 : 0;
    }
    ...
    /**
     * Get overall debt in TUSD.
     *
     * @param {FMintAccount} _fMintAccount
     * @return {number}
     */
    getOverallDebt(_fMintAccount) {
        return this.fromTokenValue(_fMintAccount.debtValue, this.tusdToken);
    }

    /**
     * Get overall collateral in TUSD.
     *
     * @param {FMintAccount} _fMintAccount
     * @return {number}
     */
    getOverallCollateral(_fMintAccount) {
        return this.fromTokenValue(_fMintAccount.collateralValue, this.tusdToken);
    }

    /**
     * Get overall borrow limit in TUSD.
     *
     * @param {FMintAccount} _fMintAccount
     * @return {number}
     */
    getBorrowLimit(_fMintAccount) {
        ...
        return this.getMaxDebtTUSD(overallCollateral) - overallDebt;
    }
    ...
    /**
     * Get overall debt limit in TUSD.
     *
     * @param {FMintAccount} _fMintAccount
     * @param {number} [_currDebtTUSD] Current debt in TUSD.
     * @param {number} [_currCollateralTUSD] Current corrateral in TUSD.
     * @return {number}
     */
    getDebtLimit(_fMintAccount, _currDebtTUSD = 0, _currCollateralTUSD = 0) {
        const overallDebt = this.getOverallDebt(_fMintAccount);
        const overallCollateral = this.getOverallCollateral(_fMintAccount);

        return this.getMintingLimitTUSD(_currDebtTUSD + overallDebt, _currCollateralTUSD + overallCollateral);
    }
     ...
     * @return {number}
     */
    getCollateralRatio(_fMintAccount, _currDebtTUSD = 0, _currCollateralTUSD = 0) {
        const overallDebt = this.getOverallDebt(_fMintAccount);
        const overallCollateral = this.getOverallCollateral(_fMintAccount);

        return (
            (overallDebt + _currDebtTUSD > 0
                ? (overallCollateral + _currCollateralTUSD) / (overallDebt + _currDebtTUSD)
                : 0) * 100
        );
    }
    ...
    canTokenBeMinted(_token) {
        // return _token && _token.isActive && _token.canMint && _token.symbol !== 'TUSD';
    ...
    canTokenBeBorrowed(_token) {
        // return _token && _token.isActive && _token.canBorrow && _token.symbol !== 'TUSD';
