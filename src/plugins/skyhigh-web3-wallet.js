...
// SkyHigh web3 wallet plugin for VUE, based on https://github.com/skyhighblockchain/skyhigh-web3-wallet
export class SkyHighWeb3Wallet {
    static install(_Vue, _options) {
        fWallet = new SkyHighWeb3Wallet(_options);
    ...
    async getTransactionToSign({ from, to, value, memo = '' }) {
        ...
        return {
            ...
            _fee: !error ? this.WEIToSKH(this.getTransactionFee(gasPrice, gasLimit)) : 0,
        };
    }
    ...
    */
    ...
    /**
     * Get the remaining balance (in SKH) after deducting transaction fee.
     *
     * @param {*} _balance
     * @param {*} _gasPrice
     * @param {string} [_gasLimit]
    ...
    getRemainingBalance(_balance, _gasPrice, _gasLimit) {
        ...
        return parseFloat(this.WEIToSKH(balance.sub(fee.mul(this.toBN(2)))));
    }
    ...
    getMaxRemainingBalance(_balance, _gasPrice) {
        ...
        return parseFloat(this.WEIToSKH(balance.sub(fee.mul(this.toBN(1)))));
    }
    ...
    /**
     * @param _address
     * @param {('skyhigh' | 'ethereum' | 'binance')} _blockchain
     * @return {boolean}
     */
    isValidAddress(_address, _blockchain = 'skyhigh') {
        if (_blockchain === 'skyhigh' || _blockchain === 'ethereum') {
