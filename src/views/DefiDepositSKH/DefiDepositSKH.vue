<template>
    <div class="view-defi-manage-collateral">
        <h1 class="with-back-btn"><f-back-button :route-name="backButtonRoute" /> Lock/Unlock {{ wskhTokenSymbol }}</h1>

        <h2 class="perex">
            Lock {{ wskhTokenSymbol }} to increase the collateral ratio and mint tUSD, unlock
            {{ wskhTokenSymbol }} after you repaid tUSD.
        </h2>
        ...
        <div class="grid">
            <div>
                <div class="df-data-item smaller">
                    ...
                    <div class="value"><f-token-value :token="wskhToken" :value="availableSKH" /></div>
                </div>
                <div class="df-data-item smaller">
                    <h3 class="label">Locked balance</h3>
                    <div class="value"><f-token-value :token="wskhToken" :value="collateral" /></div>
                </div>
            ...
            <div class="defi-price-input-col align-center">
                <div class="defi-price-input">
                    ...
                    <div class="token-label">{{ wskhTokenSymbol }}</div>
                    <div class="collateral-info">
                        <div class="collateral-info-label">Unlock {{ wskhTokenSymbol }}</div>
                        <icon data="@/assets/svg/angle-double-left.svg" width="66" height="66" aria-hidden="true" />
                    </div>
                    <div class="collateral-info increase">
                        <div class="collateral-info-label">Lock {{ wskhTokenSymbol }}</div>
            ...
            <f-message v-else-if="increasedCollateral > 0" type="info" role="alert" class="big">
                ...
                <span class="inc-desc-collateral">
                    <f-token-value :token="wskhToken" :value="increasedCollateral" no-currency /> {{ wskhTokenSymbol }}
                </span>
                to your collateral
            </f-message>
            <f-message v-else-if="decreasedCollateral > 0" type="info" role="alert" class="big">
                You’re removing
                <span class="inc-desc-collateral">
                    <f-token-value :token="wskhToken" :value="decreasedCollateral" no-currency /> {{ wskhTokenSymbol }}
                </span>
...
export default {
    ...
    computed: {
        ...
        collateral() {
            /** @type {FMintTokenBalance} */
            const tokenBalance = this.$defi.getFMintAccountCollateral(this.fMintAccount, this.wskhToken);

            return this.$defi.fromTokenValue(tokenBalance.balance, this.wskhToken) || 0;
        },
...
*/
        ...
        _maxCollateral() {
            let max = this.collateral + this.availableSKH - 1;
    ...
    methods: {
        ...
        formatInputValue(_value) {
            return parseFloat(_value).toFixed(this.$defi.getTokenDecimals(this.wskhToken));
        },

        updateMessage() {
            if (this.availableSKH <= 0.01) {
                this.message = `Deposit more ${this.wskhTokenSymbol} to increase your collateral`;
            } else {
                this.message = '';
            }
