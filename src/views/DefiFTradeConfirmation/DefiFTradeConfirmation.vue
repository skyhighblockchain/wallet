    ...
    <div class="defi-ftrade-confirmation">
        ...
        >
            ...
            <template #window-content>
                <ledger-confirmation-content :to="tx.to" :amount="amountSKH" :max-fee="tx._fee" />
            </template>
        </tx-confirmation>
        <template v-else>
...
import wskhUtils from 'skyhigh-ledgerjs/src/wskh-utils.js';
        ...
        async setTx() {
            ...
            */
            ...
            if (this.params.step === 1) {
                ...
                if (fromToken.symbol === 'SKH' && toToken.canWrapSKH) {
                    this.amountSKH = params.fromValue.toFixed(this.$defi.getTokenDecimals(params.fromToken));

                    txToSign = wskhUtils.defiWrapSKH(
                        toToken.address,
                        Web3.utils.toHex(this.$defi.shiftDecPointRight(params.toValue.toString(), toToken.decimals))
                    );
                } else if (fromToken.canWrapSKH && toToken.symbol === 'SKH') {
                    const amount = Web3.utils.toHex(
                        this.$defi.shiftDecPointRight(params.fromValue.toString(), toToken.decimals)
                    );

                    txToSign = wskhUtils.defiUnwrapSKH(
                        fromToken.address,
